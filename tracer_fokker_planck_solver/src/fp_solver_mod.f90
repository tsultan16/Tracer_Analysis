...
SUBROUTINE chang_cooper(fp2, Bsqr, divV)
    ...
    REAL(8) :: H(0:npbins+1), Wp(0:npbins+1), Wm(0:npbins+1), tau(1:npbins-1)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Dpp = Dpp * MAX(MIN(0.01D0, SQRT(Bsqr)), 10.D0) 
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !*************************************
    ! Compute H_i+1/2, Wp_i+1/2, Wm_i+1/2
    !*************************************        
    H = 0.D0
    DO ip = 1, npbins-1            
    ...
    DO ip = 1, npbins-1            
        !tau(ip) =  p(ip) / (2.D0 * (ABS(Dpp(ip)) / p(ip)) + (third * ABS(divV) + ABS(pdot_loss)) * p(ip))
        tau(ip) =  1.D0 / (third * ABS(divV) + ABS(pdot_loss)) 
    END DO
        
    ! compute required time step size with CFL number = 0.5 
    !subdt = MIN(tracer_dump_interval, 0.5D0 * MINVAL(tau) )    !  dt / DBLE(nsub) !COUR_p * pdot_max / tls%dlp  
    !PRINT*,'Required time-step size = ',subdt
   
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! compute number of subcycle steps
    nsub = 200 !2000 !20 !60 !120      
