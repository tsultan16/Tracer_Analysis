...
t5 = OMP_GET_WTIME()
   ...
   !##############################################################################################################
    ...
    WRITE(*,'(" Time elapsed = ",i3," hour ",i3," min ",i3, " sec")') INT(t9-t0)/3600 , MOD(INT(t9-t0),3600)/60, MOD(INT(t9-t0),60) 
    PRINT*,''
    ...
    DO kz = 0, nmax/2-1
        DO ky = 0, nmax/2-1
            DO kx = 0, nmax/2-1
                ...
                !*******************************************************  
                IF((kperpk_sqr/kpark_sqr) .LT. 1.d-6) THEN       ! parallel propagation
                    kperp(:) = 0.d0
                    kperp_hat(:) = 0.d0
                ELSE IF((kpark_sqr/kperpk_sqr) .LT. 1.d-6) THEN  ! perp propagation
                    kpar(:) = 0.d0
                    kpar_hat(:) = 0.d0
                END IF
...
SUBROUTINE mode_decomposition_tracer_wavelets(t_dump, wavelet_reconstruction)
    ...
    REAL*4 :: bmag, B0(3), B0_hat(3), alpha, D, zet_a(3), zet_f(3), zet_s(3), khat(3), &
              kdotB, kvec(3), kpar(3), kperp(3), kpar_hat(3), kperp_hat(3), tmp1, tmp2, tmp3, tmp4, rhok_f(2), rhok_s(2), bk_a(2), bk_f(2), bk_s(2), &
              delvk_f, delvk_s, ca, cf, cs, icf, ics, vsqr_a, vsqr_f, vsqr_s, rhosqr_f, rhosqr_s, bsqr_a, bsqr_f, bsqr_s, sound_speed  
    INTEGER :: kx, ky, kz, kbins, ik, nranks_tot, irank, ri, rj, rk, i, j, ii, isc, indx
    REAL*4 :: kmin, kmax, dk, k, kpar_sqr, kperp_sqr, kpark_sqr, kperpk_sqr, k0, dv_shell, cmplt 
    CHARACTER(LEN=300) :: filename
    CHARACTER(LEN=6) :: uniti
    REAL*4, ALLOCATABLE :: Pk_v(:,:), Pk_b(:,:), Pk_rho(:,:) 
    REAL(4) :: vw(nx,ny,nz,3), rho0, B0(3)
    ...
    ! loop over MPI ranks
    !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(irank,ri,rj,rk,i,j,isc,ii,ix,iy,iz,vw,rho0,B0,B0_hat,kx,ky,kz,k,kvec)
    !$ PRIVATE(khat,kpar,kpar_hat,kperp,kperp_hat,kpar_sqr,kperp_sqr,kpark_sqr,kperpk_sqr,alpha,D,zet_a,zet_f,zet_s)
        ...
        DO i = 1, dat_rec(ri,rj,rk)%nparticles
            ...
            DO j = 1, nwavelets_per_scalar
                ...
                DO isc = 1, 3
                    ...
                    CALL compute_basis_wavelet(ix,iy,iz,vw(:,:,:,isc))

                END DO
  
                ! get local average density and B field 
                rho0  = dat_rec(ri,rj,rk)%wavelet_avg_rhoB(wavelet_lookup_table(ii),2)   
                B0(1:3) = dat_rec(ri,rj,rk)%wavelet_avg_rhoB(wavelet_lookup_table(ii),3:5) 
                bmag = SQRT(SUM(B0**2))
                B0_hat(:) = B0(:) / bmag
                alpha = 1.d0/SUM(B0**2)
                
                ! compute FFT of this wavelet
                CALL fft_3d(vw(:,:,:,1), dat_rec(i,j,k)%wavelet_vxk_re,dat_rec(i,j,k)%wavelet_vxk_im) 
                CALL fft_3d(vw(:,:,:,2), dat_rec(i,j,k)%wavelet_vyk_re,dat_rec(i,j,k)%wavelet_vyk_im) 
                CALL fft_3d(vw(:,:,:,3), dat_rec(i,j,k)%wavelet_vzk_re,dat_rec(i,j,k)%wavelet_vzk_im) 
                ...
                DO kz = 0, nz/2-1
                    DO ky = 0, ny/2-1
                        DO kx = 0, nx/2-1
                            ...
                            ! alfven mode
                            dat_rec(i,j,k)%wavelet_va_k_re(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_re(kx,ky,kz)*zet_a(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_re(kx,ky,kz)*zet_a(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_re(kx,ky,kz)*zet_a(3)
                                                                       
                            dat_rec(i,j,k)%wavelet_va_k_im(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_im(kx,ky,kz)*zet_a(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_im(kx,ky,kz)*zet_a(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_im(kx,ky,kz)*zet_a(3)      
                            
                            ! fast mode
                            dat_rec(i,j,k)%wavelet_vf_k_re(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_re(kx,ky,kz)*zet_f(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_re(kx,ky,kz)*zet_f(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_re(kx,ky,kz)*zet_f(3) 
                                                                       
                            dat_rec(i,j,k)%wavelet_vf_k_im(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_im(kx,ky,kz)*zet_f(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_im(kx,ky,kz)*zet_f(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_im(kx,ky,kz)*zet_f(3) 
                            
                            ! slow mode
                            dat_rec(i,j,k)%wavelet_vs_k_re(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_re(kx,ky,kz)*zet_s(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_re(kx,ky,kz)*zet_s(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_re(kx,ky,kz)*zet_s(3)  
                                                                       
                            dat_rec(i,j,k)%wavelet_vs_k_im(kx,ky,kz) = dat_rec(i,j,k)%wavelet_vxk_im(kx,ky,kz)*zet_s(1) + &
                                                                       dat_rec(i,j,k)%wavelet_vyk_im(kx,ky,kz)*zet_s(2) + &
                                                                       dat_rec(i,j,k)%wavelet_vzk_im(kx,ky,kz)*zet_s(3)                 
